...
- `opencli_crawl_scrape`: local OpenCLI web reader using `opencli web read --url {url} -f json`. The CLI can be installed while Browser Bridge remains disconnected; live browser-backed reads require the Chrome/Chromium extension. Integration status only checks whether the `opencli` command exists; Browser Bridge connectivity is verified by explicit search probes or actual OpenCLI search/read execution, where disconnected Bridge state is returned as `manual_setup`.
